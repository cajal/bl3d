...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, examples, transform=None):
        ...
        # Get volumes
        volumes = (data.Stack.Volume() & [{'example_id': id_} for id_ in examples]).fetch('volume', order_by='example_id')
        self.volumes = [np.expand_dims(volume, 0) for volume in volumes] # add channel dimension

        # Get labels
        labels = (data.Stack.Label() & [{'example_id': id_} for id_ in examples]).fetch('label', order_by='example_id')
